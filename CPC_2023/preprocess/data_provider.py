...
def load_images(images_file_path, batch_size, resize_size=256, is_train=True, crop_size=224, is_cen=False,
                split_noisy=False, drop_last=False):
    ...
    if not is_train:
        ...
        if is_cen:
            ...
            transformer = transforms.Compose([ResizeImage(resize_size),
                                              ...
                                              normalize])
        if split_noisy:
            clean_images = ImageList(open(images_file_path.split('.t')[0] + '_true_pred.txt').readlines(),
                                     transform=transformer)
            noisy_images = ImageList(open(images_file_path.split('.t')[0] + '_false_pred.txt').readlines(),
                                     transform=transformer)
